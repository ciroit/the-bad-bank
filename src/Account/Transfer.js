...
function Transfer(){
    ...
    const validateAccountexists = accountNumber => fetch(`http://localhost:3001/account/exists/${accountNumber}`);

    const validateRecipientAccountNumber = (value) => {

        if(value=='')
            return 'You must enter a Recipient Account Number'

        return validateAccountexists(value)
                .then(response => response.text())
                .then(text => {
                        ...
                        if(result.isSuccess){
                            ...
                            else return 'Account doesnt exists';
    ...
    const handleSubmit = (value, {resetForm})=>{
        ...
        var recipientAccountNumber = value.recipientAccountNumber;

        fetch(`http://localhost:3001/account/transfer/${ctx.userSession._id}/${accountNumber}/${amount}/${recipientAccountNumber}`)
        .then(response => response.text())
        .then(text => {
    ...
    return (
        ...
        <Card 
            ...
            body = {(
                <>
                    <Formik
                        initialValues={{amount:'', accountNumber:'', balance:0, recipientAccountNumber: ''}}
                    ...
                    >
                        ...
                        {props => (
                            ...
                            <Form>
                                ...
                                <br/>
                                <Field className='form-control' type='text' name='recipientAccountNumber' placeholder='Enter Recipient Account' validate={validateRecipientAccountNumber} />                            
                                {props.errors.recipientAccountNumber ? <div>{props.errors.recipientAccountNumber}</div> : null}
                                <br/>
                                Amount :
                                <br/>
                                ...
                                <br/>
                                <button type="submit" className="btn btn-light" disabled={!(props.isValid && props.dirty)} >Transfer</button>
